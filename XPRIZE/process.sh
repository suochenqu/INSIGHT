...
if [ "$#" -eq 4 ]; then
    Rscript analyse_results.R "$1" all_pool_reads.csv neg_pool_reads.csv "$Zscore_thresh" "$4"
else
    Rscript analyse_results.R "$1" all_pool_reads.csv neg_pool_reads.csv "$Zscore_thresh"
fi
